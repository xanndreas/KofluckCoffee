...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.trainingCandidate.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.training-candidates.update", [$trainingCandidate->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="full_name">{{ trans('cruds.trainingCandidate.fields.full_name') }}</label>
                <input class="form-control {{ $errors->has('full_name') ? 'is-invalid' : '' }}" type="text" name="full_name" id="full_name" value="{{ old('full_name', $trainingCandidate->full_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('whatsapp') ? 'is-invalid' : '' }}" type="text" name="whatsapp" id="whatsapp" value="{{ old('whatsapp', $trainingCandidate->whatsapp) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('training_class') ? 'is-invalid' : '' }}" name="training_class_id" id="training_class_id" required>
                    @foreach($training_classes as $id => $training_class)
                        <option value="{{ $id }}" {{ ($trainingCandidate->training_class ? $trainingCandidate->training_class->id : old('training_class_id')) == $id ? 'selected' : '' }}>{{ $training_class }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ ($trainingCandidate->user ? $trainingCandidate->user->id : old('user_id')) == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
