...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.trainingClass.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.training-classes.update", [$trainingClass->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.trainingClass.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $trainingClass->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $trainingClass->description) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('start_date') ? 'is-invalid' : '' }}" type="text" name="start_date" id="start_date" value="{{ old('start_date', $trainingClass->start_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('end_date') ? 'is-invalid' : '' }}" type="text" name="end_date" id="end_date" value="{{ old('end_date', $trainingClass->end_date) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('training_category') ? 'is-invalid' : '' }}" name="training_category_id" id="training_category_id" required>
                    @foreach($training_categories as $id => $training_category)
                        <option value="{{ $id }}" {{ ($trainingClass->training_category ? $trainingClass->training_category->id : old('training_category_id')) == $id ? 'selected' : '' }}>{{ $training_category }}</option>
                    @endforeach
                </select>
