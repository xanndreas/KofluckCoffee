(function(g) { "function" === typeof define && define.amd ? define(["jquery", "datatables.net", "datatables.net-buttons"], function(k) { return g(k, window, document) }) : "object" === typeof exports ? module.exports = function(k, l) { k || (k = window); if (!l || !l.fn.dataTable) l = require("datatables.net")(k, l).$;
        l.fn.dataTable.Buttons || require("datatables.net-buttons")(k, l); return g(l, k, k.document) } : g(jQuery, window, document) })(function(g, k, l, p) {
    function m(a, b, d) {
        var c = a.createElement(b);
        d && (d.attr && g(c).attr(d.attr), d.children &&
            g.each(d.children, function(a, b) { c.appendChild(b) }), d.text && c.appendChild(a.createTextNode(d.text)));
        return c
    }

    function A(a, b) { var d = a.header[b].length,
            c;
        a.footer && a.footer[b].length > d && (d = a.footer[b].length); for (var e = 0, f = a.body.length; e < f && !(c = a.body[e][b].toString(), -1 !== c.indexOf("\n") ? (c = c.split("\n"), c.sort(function(a, b) { return b.length - a.length }), c = c[0].length) : c = c.length, c > d && (d = c), 40 < d); e++);
    ...
    function v(a) {
        ...
        g.each(a, function(b, d) {
            if (g.isPlainObject(d)) v(d);
                ...
                if (n) { var c = d.childNodes[0],
                        e, f, h = []; for (e = c.attributes.length - 1; 0 <= e; e--) { f = c.attributes[e].nodeName; var q = c.attributes[e].nodeValue; - 1 !== f.indexOf(":") && (h.push({ name: f, value: q }), c.removeAttribute(f)) }
                    e = 0; for (f = h.length; e < f; e++) q = d.createAttribute(h[e].name.replace(":", "_dt_b_namespace_token_")), q.value = h[e].value, c.setAttributeNode(q) }
        ...
        h = {
            ...
            $: function(a) {
                ...
                a.addClass || (a.hide = function() { this.style.display = "none" }, a.show = function() { this.style.display = "" }, a.addClass = function(a) { this.removeClass(a);
                    this.className += " " + a }, a.removeClass = function(a) {
                    this.className = this.className.replace(RegExp("\\s*" + a + "\\s*"),
                        " ").replace(/^\s+/, "").replace(/\s+$/, "")
                }, a.hasClass = function(a) { return !!this.className.match(RegExp("\\s*" + a + "\\s*")) });
                return a
            },
    ...
    var x = function(a, b) { b.attr("id");
        ...
        B = function(a) { var b = "Sheet1";
            a.sheetName && (b = a.sheetName.replace(/[\[\]\*\/\\\?\:]/g, "")); return b },
        s = function(a, b) { var d = b.match(/[\s\S]{1,8192}/g) || [];
        ...
        t = {
            ...
            init: function(a, b, d) { h.moviePath = i.Buttons.swfPath; var c = new h.Client;
                c.setHandCursor(!0);
    ...
    var o = {
        ...
        z = [{ match: /^\-?\d+\.\d%$/, style: 60, fmt: function(a) { return a / 100 } }, { match: /^\-?\d+\.?\d*%$/, style: 56, fmt: function(a) { return a / 100 } }, { match: /^\-?\$[\d,]+.?\d*$/, style: 57 }, { match: /^\-?£[\d,]+.?\d*$/, style: 58 }, { match: /^\-?€[\d,]+.?\d*$/, style: 59 }, { match: /^\([\d,]+\)$/, style: 61, fmt: function(a) { return -1 * a.replace(/[\(\)]/g, "") } }, { match: /^\([\d,]+\.\d{2}\)$/, style: 62, fmt: function(a) { return -1 * a.replace(/[\(\)]/g, "") } }, { match: /^[\d,]+$/, style: 63 }, { match: /^[\d,]+\.\d{2}$/, style: 64 }];
    i.Buttons.swfPath = "//cdn.datatables.net/buttons/1.2.4/swf/flashExport.swf";
    i.Api.register("buttons.resize()", function() { g.each(h.clients, function(a, b) { b.domElement !== p && b.domElement.parentNode && b.positionElement() }) });
    ...
    i.ext.buttons.csvFlash = g.extend({}, t, { className: "buttons-csv buttons-flash", text: function(a) { return a.i18n("buttons.csv", "CSV") }, action: function(a, b, d, c) { a = c._flash;
            b = y(b, c);
    ...
    i.ext.buttons.excelFlash = g.extend({}, t, {
        ...
        action: function(a, b, d, c) {
            var a = c._flash,
                ...
                d = { _rels: { ".rels": g.parseXML(o["_rels/.rels"]) }, xl: { _rels: { "workbook.xml.rels": g.parseXML(o["xl/_rels/workbook.xml.rels"]) }, "workbook.xml": g.parseXML(o["xl/workbook.xml"]), "styles.xml": g.parseXML(o["xl/styles.xml"]), worksheets: { "sheet1.xml": f } }, "[Content_Types].xml": g.parseXML(o["[Content_Types].xml"]) },
                    ...
                    for (var b = 0, c = a.length; b < c; b++) {
                        ...
                        if (!(null === a[b] || a[b] === p || "" === a[b])) {
                            ...
                            for (var n = 0, o = z.length; n < o; n++)
                                if (i = z[n], a[b].match && a[b].match(i.match)) { j = a[b].replace(/[^\d\.\-]/g, "");
                                    i.fmt && (j = i.fmt(j));
                                    j = m(f, "c", { attr: { r: d, s: i.style }, children: [m(f, "v", { text: j })] }); break }
                            j || ("number" === typeof a[b] || a[b].match && a[b].match(/^-?\d+(\.\d+)?$/) && !a[b].match(/^0\d+/) ? j = m(f, "c", { attr: { t: "n", r: d }, children: [m(f, "v", { text: a[b] })] }) : (i = !a[b].replace ? a[b] : a[b].replace(/[\x00-\x09\x0B\x0C\x0E-\x1F\x7F-\x9F]/g, ""), j = m(f, "c", { attr: { t: "inlineStr", r: d }, children: { row: m(f, "is", { children: { row: m(f, "t", { text: i }) } }) } })));
                            l.appendChild(j)
                        }
                    }
                    h.appendChild(l);
            ...
            j = 0;
            for (n = b.header.length; j < n; j++) i.appendChild(m(f, "col", { attr: { min: j + 1, max: j + 1, width: A(b, j), customWidth: 1 } }));
    ...
    i.ext.buttons.pdfFlash = g.extend({}, t, {
        ...
        action: function(a, b, d, c) {
            ...
            a.setAction("pdf");
            a.setFileName(r(c));
            s(a, JSON.stringify({ title: r(c, !1), message: "function" == typeof c.message ? c.message(b, d, c) : c.message, colWidth: g.toArray(), orientation: c.orientation, size: c.pageSize, header: c.header ? e.header : null, footer: c.footer ? e.footer : null, body: e.body }))
        },
        extension: ".pdf",
